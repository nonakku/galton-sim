...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function setupSimulation() {
        ...
        bucketBoundaries = createBucketBoundaries();
    ...
    // --------------------------
    function createBucketBoundaries() {
        if (bottomRowPins.length < 1) return [];

        let boundaries = [binLeftBoundary];
        for (let i = 0; i < bottomRowPins.length - 1; i++) {
             boundaries.push((bottomRowPins[i] + bottomRowPins[i+1]) / 2);
        }
        boundaries.push(binRightBoundary);

        return boundaries;
    }

    function updateBucketCounts(forceUpdate = false) {
        const boundaries = bucketBoundaries;
        ...
        if (!forceUpdate && countsKey === lastBucketCountsKey) return;
        lastBucketCountsKey = countsKey;
    ...
    Events.on(engine, 'afterUpdate', function () {
        ...
        if (currentTime - lastBucketUpdateTime < bucketUpdateIntervalMs) return;

        lastBucketUpdateTime = currentTime;
        updateBucketCounts();
    });
